...
of Thomas Stratmann @stratmann-2002, who exploits the time series nature of
...
Regarding campaign contribution, however, @stratmann-2002 uses the aggregate
...
reports the results and chapter 7 provides the discusses the results and
concludes the paper.
...
be introduced. Political Scientist Simon Weschle defines three types of money in
...
contributors following both methods @Weschle_2022c pp.26-28.
...
donations @Weschle_2022c, @stratmann-2017 p.8. p.25. The partisan lean of a
...
and voting @stratmann-2005 p.143-144 @stratmann-2002. Considering a closer
...
preconditions stipulated by Stratmann @stratmann-2002, analysing environmental
...
package's functions fuzzy_match and fuzzy_join functions as shown in the code
block in @rollcall. Finally, the two merged dataframes were concatinated.
...
anti-environmental campaign contributions would be minimal, if statistically
significant. As can be seen from the regression results in the appendix, this
was the case.// From using control variables to state and year fixed effects in
// the LPM with all representatives, campaign contributions from environmental and
// non-environmental sources were highly significant.

// LPM all representatives with control variables & log transformed
In the simplest LPM referenced in @lpm, incorporating control variables revealed
that a one-dollar increase in pro-environmental contributions to representatives
marginally increases the likelihood of a pro-environmental vote by an average of
0.00007214 percent, assuming all other factors remain constant. Conversely, a
one-dollar increase in anti-environmental contributions reduces the probability
of a pro-environmental vote by an average of 0.000006 percent, ceteris paribus.
Both coefficients are highly statistically significant at the 0 level.

Due to the highly skewed distribution of both anti-environmental and
pro-environmental contributions, I applied a logistic transformation to these
variables. This adjustment improved the adjusted $R^2$
from 0.91 to 0.92. After the transformation, the influence of the
anti-environmental contributions diminished to -0.0121, though it retained the
same level of statistical significance. In contrast, the effect of the
pro-environmental contributions was measured at an average of 0.011; however,
the significance level of this estimator decreased to $p < 0.01$. This pattern
persisted when controlling for state and year, as well as legislator and year.
Given that the results are less significant after logistic transformation
compared to the non-transformed model, the discussion and interpretation will
focus exclusively on the non-transformed results from here onward.// check this for ind& year FE!

// fixed effects
When fixing the LPM model by US year and, alternatively: party, geographical
regions or state, all three fixed effects model show similar results. The
adjusted $R^2$ remains at 0.91 and the contribution coefficients remain highly
significant with a one USD increase in pro-environmental contribution increasing
the probability of a pro-environmental vote by 0.00007 percent and a one USD
increase in anti-environmental contribution decreasing the probability of a
pro-environmental vote by 0.000005 percent, ceteris paribus.Only when applying
legislator and year fixed effects does the significance of the pro-environmental
contributions decrease to a 0.01 level, with a one USD increase in
pro-environmental contribution increasing the probability of a pro-environmental
vote by 0.0000361 percent on average, ceteris paribus. The anti-environmental
contribution coefficient however is not significant at all. Despite this, the
model remains highly effective in explaining variations in the dependent
variable, achieving an adjusted $R^2$ of 0.953.

// Interpretation of coefficients
The aforementioned results corraborate the first hypothesis. Not only are the
results highly significant but the effects are also very small. It is important
to put these results into context, however. Since the contribution coefficients
are estimated per USD, these small effects become quite large, depending on the
average contribution per representative.

//  include plot / info about average contributions per session? (show boxplots of contributions)

Given that representatives receive less pro-environmental contributions than
anti enviornmental contributions, and their contributions are also estimated to
be more effective in ensuring a pro environmental vote -> this evens out?
