...
filter_session_data_2 <- function(df, vote) {
    if (vote == 3) {
        ...
        selected_cols <- c(
            ...
            "Vote3", "Contribution_3_minus", "Contribution_3_plus", "state", "BioID"
        )
    ...
    if (vote == 4) {
        selected_cols <- c(
            "party", "Vote_change_dummy",
            "Vote4", "Contribution_4_minus", "Contribution_4_plus", "state"
        )
        df <- df[, c(selected_cols)]
    ...
    if (vote == 51) {
        selected_cols <- c(
            "party", "Vote_change_dummy",
            "Vote51", "Contribution_51_minus", "Contribution_51_plus", "state"
        )
        df <- df[, c(selected_cols)]
    ...
    if (vote == 52) {
        selected_cols <- c(
            "party", "Vote_change_dummy",
            "Vote52", "Contribution_52_minus", "Contribution_52_plus", "state"
        )
        df <- df[, c(selected_cols)]
    ...
    if (vote == 6) {
        selected_cols <- c(
            "party", "Vote_change_dummy",
            "Vote6", "Contribution_6_minus", "Contribution_6_plus", "state"
        )
        df <- df[, c(selected_cols)]
    ...
    if (vote == 7) {
        selected_cols <- c(
            "party", "Vote_change_dummy",
            "Vote7", "Contribution_7_minus", "Contribution_7_plus", "state"
        )
        df <- df[, c(selected_cols)]
        # view(df)
    }
