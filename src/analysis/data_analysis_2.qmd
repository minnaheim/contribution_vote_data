...
```{r}
df_sub <- df_sub %>% select(-c(BioID, GovtrackID, name.x, name.y, first_vote, Vote_change, Vote_count, Vote_change_dummy, first_name, last_name, district, Vote3, Vote4, Vote51, Vote52, Vote6, Vote7, state, vote_change_year, opensecrets_id))
df_sub <- df_sub %>% filter(!is.na(first_contribution_minus))
