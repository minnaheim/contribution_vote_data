...
```{r}
kable(df_vote_change[1:25, ], format = "html")
```
...
### checking differences in party
```{r}
ols_change_D <- lm(Vote ~ . - state - party - BioID - district - anti_env_dummy, data = df_long_change)
summary(ols_change_D)
ols_change_R <- lm(Vote ~ . - state - party - BioID - district - anti_env_dummy, data = df_long_change)
