...
```{r}
ols_ind_fe <- lm(Vote ~ district + party + birthday + nominate_dim1 + nominate_dim2 + as.factor(Instance) + as.factor(BioID) + seniority + Contribution_minus + Contribution_plus + Dmajority, data = df_long)
summary(ols_ind_fe)
...
### with control variables
```{r}
full_logit <- glm(Vote ~ . - state - BioID - Vote_change_dummy, family = binomial(link = "logit"), data = df_long)
summary(full_logit)
```
...
full_logit_log <- glm(Vote ~ . - state - BioID - Vote_change_dummy, family = binomial(link = "logit"), data = df_long)
