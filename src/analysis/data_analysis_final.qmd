...
ols_party_dummy <- lm(Vote ~ district + birthday + nominate_dim1 + nominate_dim2 + Geographical + as.factor(party) + as.factor(Instance) + seniority + gender + pro_env_dummy + anti_env_dummy, data = df_long)
...
## LPM models 
```{r}
main_ols <- stargazer(full_ols, ols_party_plm, ols_geo_plm, ols_ind_fe_plm, ols_state_fe_plm, type = "text")
basic_ols <- stargazer(full_ols_dummy, full_ols, full_ols_log, type = "text")
# party_ols <- stargazer(ols_party, ols_party_dummy, type = "text")
# geo_ols <- stargazer(ols_geo, ols_geo_log, type = "text")
