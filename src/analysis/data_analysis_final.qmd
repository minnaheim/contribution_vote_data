...
df_long <- df_long %>% select(-c(BioID, GovtrackID, opensecrets_id, first_name, last_name, state, name))
view(df_long)
full_ols <- lm(Vote ~ ., data = df_long)
summary(full_ols)
```

The ols regresses the pro and anti environmental contributions with the control variables used: party, district, birthday, seniority (number of terms served prior to vote), the DW-Nominate Scores from the 1st and second dimensions, geographical region (i.e. States categorised into regions (South, West, North-East and Midwest)), and the majority party in the house at the time of the vote.
...
full_ols_log <- lm(Vote ~ ., data = df_long)
...
df_long_D <- df_long %>%
    ...
    select(-party)
View(df_long)
full_ols_D <- lm(Vote ~ ., data = df_long)
summary(full_ols_D)
...
df_long_R <- df_long %>%
    ...
    select(-party)
View(df_long)
full_ols_R <- lm(Vote ~ ., data = df_long)
summary(full_ols_R)
```
...
```{r}
ols_state_fe <- plm(Vote ~ .,
    data = df_long, index = c("Geographical", "Instance"), model = "within",
    effect = "twoways"
)
